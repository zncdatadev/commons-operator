...
func TestE2E(t *testing.T) {
	...
	_, _ = fmt.Fprintf(GinkgoWriter, "Starting commons-operator integration test suite\n")
...
var _ = BeforeSuite(func() {
	...
	cmd = exec.Command("make", "docker-build", fmt.Sprintf("IMG=%s", utils.GetProjectImg()))
	_, err = utils.Run(cmd)
	ExpectWithOffset(1, err).NotTo(HaveOccurred(), "Failed to build the manager(Operator) image")

	By("loading the manager(Operator) image on Kind")
	err = utils.LoadImageToKindClusterWithName(utils.GetProjectImg())
